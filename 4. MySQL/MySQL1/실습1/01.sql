...
FROM customers
WHERE country IN ('USA') AND state in ('CA')
ORDER BY customerName DESC

-- 문제 6
SELECT customerNumber , customerName , country , state
FROM customers
WHERE country IN ('USA') AND state in ('CA', 'NY') 
ORDER BY customerNumber DESC
...
SELECT productCode , productName , quantityInStock
FROM products
WHERE quantityInStock < '3000' AND quantityInStock > '2000'
...
FROM employees
GROUP BY jobTitle
ORDER BY COUNT(*) DESC, jobTitle DESC
...
-- 문제 15
SELECT LEFT(orderDate, 4) AS year, COUNT(orderNumber)
FROM orders
